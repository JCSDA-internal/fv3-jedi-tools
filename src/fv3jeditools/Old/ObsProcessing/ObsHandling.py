...
# --------------------------------------------------------------------------------------------------
    ...
    def setup(self):
        ...
        self.process_date = dt.datetime.strptime(
            self.process_date_str, '%Y%m%d%H')

        # Observation file
        self.obstarfile = self.process_date.strftime(
            self.config['obs_fileformat'])
...
# --------------------------------------------------------------------------------------------------
    ...
    def downloadObsS3(self):
        ...
        # Retrieve from S3
        utils.recvS3(self.config['obs_targetpath'],
                     self.obstarfile, self.config['obs_sourcepath'])
...
# --------------------------------------------------------------------------------------------------
    ...
    def downloadObsArchive(self):
        ...
        remote_path_file = os.path.join(
            self.config['obs_targetpath'], self.obstarfile)
