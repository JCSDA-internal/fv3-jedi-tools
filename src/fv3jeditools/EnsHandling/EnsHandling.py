    ...
    def downloadGeosEnsRestartArchive(self):
        ...
        for n in range(1, self.nEns+1):
            ...
            for ens_file in self.cf['ens_files']:
                ...
                # Split off extension
                ens_tarfile_nam, ens_tarfile_ext = os.path.splitext(
                    ens_tarfile)

                # File name with correct date
                ens_int_file = os.path.join(
                    ens_tarfile_nam, memstr, self.winbeg_date.strftime(ens_file))
                ...
                if remote_file_size == -1:
                    utils.abort("EnsembleHandling.downloadGeosEnsRestartArchive restart file " +
                                ens_int_file+" not available in restart tar")

                # Local file
                local_path_file = os.path.join(
                    self.cf['ens_targetpath'], ens_int_file)
                ...
                if local_file_size != remote_file_size:
                    print("downloadGeosRestartArchive: getting restart ",
                          ens_int_file)
                    tf.extractall(self.cf['ens_targetpath'],
                                  members=tarmembers[n:n+1])
                else:
                    print(
                        "downloadGeosRestartArchive: already have restart ", ens_int_file)
