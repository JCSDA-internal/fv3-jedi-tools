  ...
  def downloadGeosRestartArchive(self):
    ...
    for rst_file in self.cf['rst_files']:
      ...
      if remote_file_size == -1:
        utils.abort("BackgroundHandling.downloadGeosRestartArchive restart file "+rst_int_file+" not available in restart tar")

      # Local file
      local_path_file = os.path.join(self.cf['rst_targetpath'],rst_int_file)
      ...
      if local_file_size != remote_file_size:
        ...
        tf.extractall(self.cf['rst_targetpath'],members=members[n:n+1])
      else:
...
# --------------------------------------------------------------------------------------------------
  ...
  def getGeosBackgrounds(self):
    ...
    dts = utils.getDateTimes(start,final,float(self.cf['timestep']),dtformat)
...
# TODO dmget the files first
    ...
    for dt in dts:
      ...
      bkg_path_file_target = os.path.join(self.cf['bkg_targetpath'],bkg_file)
      ...
      if source_file_size == -1:
        utils.abort("BackgroundHandling.getGeosBackgrounds background file "+bkg_path_file_source+" not available")
