...
if test "${run_daily_state_to_control}" = "true"; then
   ...
   for yyyymmddhh in ${yyyymmddhh_list}; do
      run_sbatch state_to_control_${yyyymmddhh}.sh ""
...
if test "${run_daily_vbal}" = "true"; then
   daily_vbal_pids=""
   for yyyymmddhh in ${yyyymmddhh_list}; do
      run_sbatch vbal_${yyyymmddhh}.sh ${daily_state_to_control_pids}
...
# Run dirac_full_c2a_global
if test "${run_dirac_full_c2a_global}" = "true"; then
   run_sbatch dirac_full_c2a_global_${yyyymmddhh_first}-${yyyymmddhh_last}.sh ${merge_nicas_pid}${merge_states_pid}${final_vbal_pid}
   dirac_full_c2a_global_pid=:${pid}
fi

# Run dirac_full_regrid_local
if test "${run_dirac_full_regrid_local}" = "true"; then
   run_sbatch dirac_full_c${cregrid}_${nlx_regrid}x${nly_regrid}_local_${yyyymmddhh_first}-${yyyymmddhh_last}.sh ${regrid_merge_nicas_pid}${regrid_vbal_pid}${regrid_states_pid}
...
if test "${run_variational_3dvar}" = "true"; then
   run_sbatch variational_3dvar_${yyyymmddhh_first}-${yyyymmddhh_last}.sh ${merge_nicas_pid}${merge_states_pid}${final_vbal_pid}
...
if test "${run_variational_3dvar_specific_obs}" = "true"; then
   ...
   for obs in ${obs_xp} ; do
      run_sbatch variational_3dvar_${obs}_${yyyymmddhh_first}-${yyyymmddhh_last}.sh ${merge_nicas_pid}${merge_states_pid}${final_vbal_pid}
...
if test "${run_variational_3dvar_regrid}" = "true"; then
   run_sbatch variational_3dvar_c${cregrid}_${nlx_regrid}x${nly_regrid}_${yyyymmddhh_first}-${yyyymmddhh_last}.sh ${regrid_merge_nicas_pid}${regrid_vbal_pid}${regrid_states_pid}
   variational_3dvar_regrid_pid=:${pid}
fi

# Run 3dvar_full_regrid
if test "${run_variational_3dvar_full_regrid}" = "true"; then
   run_sbatch variational_3dvar_full_c${cregrid}_${nlx_regrid}x${nly_regrid}_${yyyymmddhh_first}-${yyyymmddhh_last}.sh ${regrid_merge_nicas_pid}${regrid_vbal_pid}${regrid_states_pid}
