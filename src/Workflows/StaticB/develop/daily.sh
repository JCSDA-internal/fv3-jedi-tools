...
for yyyymmddhh in ${yyyymmddhh_list}; do
  ...
  mkdir -p ${data_dir_def}/var-mom_${yyyymmddhh}${rr}
done
...
background error:
  ...
  output ensemble:
    filetype: fms restart
    datapath: ${data_dir_def}/${yyyymmddhh}${rr}/mem%{member}%/
    prepend files with date: false
  ...
  saber central block:
    ...
    calibration:
      ...
      io:
        ...
        files prefix: var-mom_${yyyymmddhh}${rr}/var-mom_${yyyymmddhh}${rr}
      drivers:stateVars
